...
export function* range(start: number, end?: number, step: number = 1): Iterable<number> {
    ...
    if (end === undefined) {
        ...
        if (end < start)
            yield* sequence(
                iterate(subtract(absolute), start),
                takeWhile(n => n >= end)
            );
        else
            yield* sequence(
                iterate(add(absolute), start),
                takeWhile(n => n <= end)
            );
    }
}
...
export function sequence(
    source: IterableLike<any> | AsyncIterable<any>,
    ...transducers: Transducer<any, any>[]
): Sequence<any> | AsyncIterable<any> {
...
export class Sequence<T> implements Iterable<T> {
    constructor(
        public source: Iterable<any>,
        public transducers: Transducer<any, any>[]
    ) {}
...
export class AsyncSequence<T> implements AsyncIterable<T> {
    constructor(
        public source: AsyncIterable<any>,
        public transducers: Transducer<any, any>[]
    ) {}
