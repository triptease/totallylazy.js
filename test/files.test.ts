...
describe("files", function () {
  ...
  it("can return name", function () {
    assert.strictEqual(
      new File("example/child.txt", __dirname).name,
      "child.txt"
    );
  });

  it("can list children", async () => {
    const size = (await array(new File("example/", __dirname).children()))
      .length;
  ...
  it("can tell if file exists", async () => {
    assert(
      !(await new File("some-random-file-that-does-not-exist", __dirname)
        .exists)
    );
  });

  it("can list descendants", async () => {
    const size = (await array(new File("example/", __dirname).descendants()))
      .length;
